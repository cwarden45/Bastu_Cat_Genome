...
**Update (6/25)**: The CEO of basepaws was kind enough to call me today.  I think part of the "first" part is in the use of the word "consumer".  Because UC-Davis is a non-profit organization, that is why they didn't consider the UC-David VGL to provide the first test (for Ancestry or [other markers](https://www.vgl.ucdavis.edu/services/cat/)).
...
**c)** I thought they had their own HiSeq-X (although was not sure how they would have one machine), but this is incorrect.  They prepare libraries and sequencing is done by a 3rd party.  They have MiniSeq in the lab that they can use for some R&D testing, but that is not used for the CatKits.  Hopefully, they are at least getting entire lanes with just their samples, but I am trying to learn more about how/if Illumina machine throughput affects de-multiplexing (and I'm have some notes about the reads for my basepaws sample [here](https://github.com/cwarden45/Bastu_Cat_Genome/tree/master/Basepaws_Notes/Read_QC))
