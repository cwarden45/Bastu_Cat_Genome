...
As mentioned in [a comment to this Instagram post](https://www.instagram.com/p/Bu1uXAsAI6J/?fbclid=IwAR3TgmQ-wVweKm9bmzZFVgP-6vJAjfq8jeEMALVuwQa_XvO4cxtmgJos9Uw), "*we use 0.5x coverage for the calculation of the breed index only. For health markers we will be using targeted capture, single base extension sequencing with higher coverage or sanger based sequencing.*"  Likewise, I have this [blog post](http://cdwscience.blogspot.com/2019/08/low-coverage-sequencing-is-not.html) describing limitations to low coverage sequencing in humans.  So, that trait information can't be provided until they do those additional experiments, if they are done (and I think they may need to only provide the broader "Western-Eastern-Hybrid-Polycat" ancestry results in the future).
...
**Update (6/25)**: The CEO of basepaws was kind enough to call me today.  The intention of the word "first" is in respect to a "*consumer*" test.  Because UC-Davis is a non-profit organization, that is why they didn't consider the UC-Davis VGL to provide the first test (for Ancestry or [other markers](https://www.vgl.ucdavis.edu/services/cat/)).
...
**b)** The Facebook discussion group should probably be updated to use the Western-Eastern-Hybrid-Polycat percentages (rather than the specific percentages), and I like their "**One simple cat DNA test, a lifetime of discovery**" catchphrase more than "*Most Accurate Cat DNA Test*" (which I think is harder to define than you might imagine, and/or may not be true - plus, my understanding from a basepaws staff member if that the Sphyinx breed was determined from one locus, so I don't believe it makes sense to be 5% sphynx).

**c)** I thought they had their own HiSeq-X (although was not sure how they would have one machine), but I think I misunderstood this.  They prepare libraries and sequencing is done by a 3rd party.  They have MiniSeq in the lab that they can use for some R&D testing, but that is not used for the CatKits.  Hopefully, they are at least getting entire lanes with just their samples, but I am trying to learn more about how/if Illumina machine throughput affects de-multiplexing (and I'm have some notes about the reads for my basepaws sample [here](https://github.com/cwarden45/Bastu_Cat_Genome/tree/master/Basepaws_Notes/Read_QC))

I really hope I am not interpreted as being too negative: I think some things may need to be changed, but I very much hope and expect to see future changes with the same group of individuals.  So, I certainly want the members of basepaws to succeed and help with cat research.  I am also working out some details for my future workload.  So, I have also made these types of mistakes, but it is important to take the correction of mistakes into consideration when planning for time that needs to be taken for a project in the future.
