...
**c)** I thought they had their own HiSeq-X (although was not sure how they would have one machine), but this is incorrect.  They prepare libraries and sequencing is done by a 3rd party.  They have MiniSeq in the lab that they can use for some R&D testing, but that is not used for the CatKits.  Hopefully, they are at least getting entire lanes with just their samples, but I am trying to learn more about how/if throughput affects base calling (and I'm have some notes about the reads for my basepaws sample [here](https://github.com/cwarden45/Bastu_Cat_Genome/tree/master/Basepaws_Notes/Read_QC))
