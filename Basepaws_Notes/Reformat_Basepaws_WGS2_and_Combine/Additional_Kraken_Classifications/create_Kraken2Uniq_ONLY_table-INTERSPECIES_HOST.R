input_folder = "Kraken-OUT-minikraken_8GB_20200312-Minimizer"
...
output.classification_rate = "n29_Kraken2Uniq-Bacterial_Classifications.txt"
output.percent_quantified = "n29_FILTERED_Kraken2Uniq_genera-heatmap_quantified.txt"
output.counts = "n29_FILTERED_Kraken2Uniq_genera-counts.txt"
heatmap.output_quantified = "n29_FILTERED_Kraken2Uniq_genera-heatmap_quantified.png"
heatmap.output_quantified20 = "n29_FILTERED_Kraken2Uniq_genera-heatmap_quantified-TOP20.png"
min.percent = 0.5
...
for (i in 1:length(Kraken2_files)){
	...
	classification.rate[i]=Kraken2_table$V1[Kraken2_table$V8 == "    Bacteria"]
...
for (i in 1:length(Kraken2_files)){
	...
	print(sampleID)
	Kraken2_table = read.table(paste(input_folder,Kraken2_files[i],sep="/"), head=F, sep="\t")
	Kraken2_table$V8 = gsub("\\s+","",as.character(Kraken2_table$V8))
	print(dim(Kraken2_table))##MODIIFED CODE
	Eukaryota_index = grep("^Eukaryota$",Kraken2_table$V8)#MODIFIED CODE
	Bacteria_index = grep("^Bacteria$",Kraken2_table$V8)#MODIFIED CODE
	if(length(Eukaryota_index) == 1){
		if(length(Bacteria_index) == 1){
			if(Bacteria_index > Eukaryota_index){
				...
				Viral_index = grep("^Viruses$",Kraken2_table$V8)#MODIFIED CODE
				if(length(Viral_index) == 1){
					...
					print(Kraken2_table$V8[Viral_index])#MODIFIED CODE
					stop()#MODIFIED CODE
				}#end 	
				Kraken2_table=Kraken2_table[keep_rows,]#MODIFIED CODE
			...
			print(Kraken2_table$V8[Bacteria_index])#MODIFIED CODE
			stop()#MODIFIED CODE
		}#end else...MODIFIED CODE
		...
		print(Kraken2_table$V8[Eukaryota_index])#MODIFIED CODE
		stop()#MODIFIED CODE
	}#end 

	print(dim(Kraken2_table))##MODIIFED CODE

	Genera_table = Kraken2_table[Kraken2_table$V6 == "G",]
	Genera_table$V8 = gsub("\\s+","",as.character(Genera_table$V8))
	Genera_table = Genera_table[!is.na(Genera_table$V8),]
	Genera_table = Genera_table[Genera_table$V8 != "",]
	sample_genera = gsub("\\s+","",as.character(Genera_table$V8))
