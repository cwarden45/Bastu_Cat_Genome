...
output.percent_quantified = "test1-n8_Oral_Kraken2_Braken-direct_quantification.txt"
dot_plot.output = "test1-n8_Oral_Kraken2_Braken-direct_quantification-Staphylococcus.png"
heatmap.output = "test1-n8_Oral_Kraken2_Braken-direct_quantification-heatmap_n11.png"
...
non_host.counts = apply(oral_table, 2, sum)

percent_table=oral_table
for (i in 1:ncol(percent_table)){
	percent_table[,i] = round(100 * oral_table[,i]/non_host.counts[i], digits=4)
...
plot(as.numeric(as.factor(Samples)), as.numeric(percent_table[rownames(percent_table)=="Streptococcus",]),
		...
		xlab="", ylab="Percent Non-Host (Streptococcus)", pch=16, col=speciesCol,
		cex=1.2, main="")
