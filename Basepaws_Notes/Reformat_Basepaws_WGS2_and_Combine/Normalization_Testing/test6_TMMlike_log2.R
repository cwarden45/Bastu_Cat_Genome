...
output.percent_quantified = "test6-n8_Oral_Kraken2_Braken-TMMlike_log2.txt"
dot_plot.output = "test6-n8_Oral_Kraken2_Braken-TMMlike_log2-Staphylococcus.png"
heatmap.output = "test6-n8_Oral_Kraken2_Braken-TMMlike_log2-heatmap_n11.png"
...
for (i in 1:ncol(percent_table)){
	...
	percent_table[,i] = round(log2(100 * temp_counts/temp_quantile_sum + 0.5), digits=2)#could be >100%
...
plot(as.numeric(as.factor(Samples)), as.numeric(percent_table[rownames(percent_table)=="Streptococcus",]),
		...
		xlab="", ylab="log2(TMM-like Percent + 0.5) (Streptococcus)", pch=16, col=speciesCol,
		cex=1.2, main="")
