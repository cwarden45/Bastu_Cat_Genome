...
output.percent_quantified = "test3-n8_Oral_Kraken2_Braken-log2_median.txt"
dot_plot.output = "test3-n8_Oral_Kraken2_Braken-log2_median-Staphylococcus.png"
heatmap.output = "test3-n8_Oral_Kraken2_Braken-log2_median-heatmap_n11.png"
...
median_temp_table = oral_table
median_temp_table[median_temp_table == 0]=NA
non_zero.median = apply(median_temp_table, 2, median, na.rm=T)
all.median = apply(oral_table, 2, median)

oral_table = oral_table + 1 #avoid taking log2 of 0 at a later step

norm_table=matrix(ncol=ncol(oral_table), nrow=nrow(oral_table))
colnames(norm_table)=colnames(oral_table)
rownames(norm_table)=rownames(oral_table)

for (i in 1:ncol(norm_table)){
	norm_table[,i] = round(log2(oral_table[,i]/non_zero.median[i]), digits=4)
}#end for (i in 2:ncol(genus_percent.normalize_external))

write.table(data.frame(Genus=rownames(norm_table), norm_table), output.percent_quantified, quote=F, sep="\t", row.names=F)
...
par(mar=c(15,5,5,2))
plot(as.numeric(as.factor(Samples)), as.numeric(norm_table[rownames(norm_table)=="Streptococcus",]),
...
heatmap.abundance = norm_table[match(filtered_genera, rownames(norm_table)),]
