...
heatmap.file = "mm10_Bowtie2-FLASH_Bowtie2_alignment_rate_per_chr.pdf"
...
for (i in 1:length(idxstat_files)){
	...
	if(sum(idxstat_table$V3) != 0){
		temp_percentage = 100 * idxstat_table$V3 / sum(idxstat_table$V3)
		temp_percentage = temp_percentage[match(chrs,idxstat_table$V1)]
	}else{
		temp_percentage = rep(0,length(chrs))
	}#end else
