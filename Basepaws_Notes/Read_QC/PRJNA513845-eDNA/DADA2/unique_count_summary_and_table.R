seqFolder = "FLASH"
seqType = "FLASH_merged"
metaIn = "Cutadapt-filtered_read_counts-with_sequencer.txt"
metaOut =  "Cutadapt-filtered_read_counts-with_sequencer_and_FLASHcounts.txt"
countOut =  "FLASH_merged_read_counts-greater_than_1_in_10000.txt"
lengthPlot =  "FLASH_merged_read_length_distribution.png"
lengthOut =  "FLASH_merged_read_length_distribution.txt"
min_plot_length=200
max_plot_length=300
...
outliers =  c("SRR8423822","SRR8423819","SRR8423847","SRR8423820",
				"SRR8423878")#add SRR8423878 because FLASH was run but there were no remaining reads

meta.table = read.table(metaIn, head=T, sep="\t")
print(dim(meta.table))
meta.table = meta.table[-match(outliers,meta.table$Sample),]
print(dim(meta.table))
