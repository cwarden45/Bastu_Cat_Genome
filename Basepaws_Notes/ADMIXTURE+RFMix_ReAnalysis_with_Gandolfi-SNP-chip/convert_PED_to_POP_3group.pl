...
my $mapping_file = "../FID_breed_codes_fromHasan.txt";#if you want to change the mapping, you'll have to modify this code (and either over-write the other file, or save the files in another location)
...
while (<INPUTFILE>){
	...
	if($line_count > 1){
		...
		if($ancestry eq "Eastern"){
			...
			$ancestry_hash{$FID}=$ancestry;
		}elsif($breed eq "Domestic"){
			$ancestry_hash{$FID}="Mixed";
		}else{
			print "Skipping: $line\n";
		}
